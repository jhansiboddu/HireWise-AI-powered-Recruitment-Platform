...
//       try {
//         const res = await axios.get("/shortlist/shortlisted/all", {
...
//         const parsedCandidates = Array.from(listItems).map((li) => {
//           const text = li.innerText;
//           // Format: "candidateId - name - email - Score: score - Job ID: jobId"
//           const match = text.match(/^(\d+)\s*-\s*(.+?)\s*-\s*(.+?)\s*-\s*Score:\s*(\d+\.?\d*)\s*-\s*Job ID:\s*(\d+)/);
//           return {
//             candidateId: match?.[1],
//             name: match?.[2],
//             email: match?.[3],
...
//         setCandidates(parsedCandidates);
//       } catch (err) {
//         console.error("Error fetching all shortlisted candidates:", err);
//       }
//     };
...
//   const handleSendEmail = async (candidateId, jobId) => {
//     try {
//       await axios.post(
//         "/shortlist/send_email/",
//         new URLSearchParams({
...
//       setCandidates((prev) =>
//         prev.map((c) =>
//           c.candidateId === candidateId && c.jobId === jobId
//             ? { ...c, emailSent: true }
//             : c
//         )
//       );
...
//                 <button
//                   className="send-email-btn"
//                   onClick={() => handleSendEmail(c.candidateId, c.jobId)}
//                   disabled={c.emailSent}
//                 >
//                   {c.emailSent ? "Email Sent ✅" : "Send Email"}
//                 </button>
...
// export default AllShortlistedCandidates;
...
const AllShortlistedCandidates = () => {
  ...
  useEffect(() => {
    const fetchAllShortlisted = async () => {
      try {
        ...
        const parsedCandidates = Array.from(listItems)
          .map((li) => {
            const text = li.innerText;
            const match = text.match(
              /^(\d+)\s*-\s*(.+?)\s*-\s*(.+?)\s*-\s*Score:\s*(\d+\.?\d*)\s*-\s*Job ID:\s*(\d+)/
            );
            ...
            return {
              ...
              jobId: parseInt(match[5]),
            };
          })
          .filter(Boolean) // remove nulls
          .sort((a, b) => a.jobId - b.jobId); // sort by jobId ascending
  ...
  // Filtered list based on searchJobId
  const filteredCandidates = candidates.filter((c) =>
    searchJobId === "" ? true : c.jobId.toString().includes(searchJobId)
  );
